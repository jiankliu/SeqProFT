...
from esm_next.tokenization import EsmSequenceTokenizer
from esm_next.utils import encoding
from esm_next.utils.constants.models import ESMC_600M
from esm_next.utils.decoding import decode_sequence
from esm_next.utils.misc import stack_variable_length_tensors
from esm_next.utils.sampling import _BatchedESMProteinTensor
...
class ESMC(nn.Module, ESMCInferenceClient):
    ...
    def forward(
        ...
        x, _, hiddens, attentions, attentions_raw = self.transformer(x, sequence_id=sequence_id)
        sequence_logits = self.sequence_head(x)
        # output = ESMCOutput(
        #     sequence_logits=sequence_logits, embeddings=x, hidden_states=hiddens
        # )
        return sequence_logits, x, hiddens, attentions, attentions_raw
