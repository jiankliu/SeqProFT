...
from esm_next.models.esm3 import ESM3
...
from esm_next.utils.constants.esm3 import data_root
...
def ESMC_300M_202412(device: torch.device | str = "cpu"):
    ...
    state_dict = torch.load(
        "dwnl_ckpts/esmc_300m_2024_12_v0_lora.pth",
        map_location=device,
    )
    model.load_state_dict(state_dict)
...
def ESMC_600M_202412(device: torch.device | str = "cpu"):
    ...
    state_dict = torch.load(
        "dwnl_ckpts/esmc_600m_2024_12_v0_lora.pth",
        map_location=device,
    )
    model.load_state_dict(state_dict)
